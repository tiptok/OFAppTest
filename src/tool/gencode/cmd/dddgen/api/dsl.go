...
func RunApiDSL(ctx *cli.Context) {
	...
	path := path{
		...
		Summary:  "测试",
		Content:  "json",
		Request:  refObject{RefPath: strings.Join([]string{constant.ApiProtocol, common.LowFirstCase(controller.Controller), RequestProtocol + ".json"}, "/")},
		Response: refObject{RefPath: strings.Join([]string{constant.ApiProtocol, common.LowFirstCase(controller.Controller), ResponseProtocol + ".json"}, "/")},
	}
